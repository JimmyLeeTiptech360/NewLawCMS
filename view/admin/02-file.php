   ...
   $(document).ready(function(){  
    ...
    $( "#search_button" ).click(function() {
        $.fn.dataTable.ext.search.push(
            ...
            {
                var textValue = $('#text_value').val();
                var targetId = $('#selected_target option:selected').val();               
                var targetValue = data[targetId];
                var targetIndex = targetValue.toLowerCase().indexOf(textValue.toLowerCase());
    ...
    $( "#search_button_by_range" ).click(function() {
        $.fn.dataTable.ext.search.push(
            function( settings, data, dataIndex ) 
            {
                ...
                var min = parseInt( $('#text_value_min').val(), 10 );
                var max = parseInt( $('#text_value_max').val(), 10 );
                var targetId = $('#selected_target option:selected').val();               
                var targetValue = data[targetId];
                
                if ( ( isNaN( min ) && isNaN( max ) ) ||
                    ( isNaN( min ) && targetValue <= max ) ||
                    ( min <= targetValue   && isNaN( max ) ) ||
                    ( min <= targetValue   && targetValue <= max ) )
                {
                    return true;
                }
                return false;
    ...
    $('#min_id, #max_id').keyup( function() {
           $.fn.dataTable.ext.search.push(
            function( settings, data, dataIndex ) 
            {
                var min = parseInt( $('#min_id').val(), 10 );
                var max = parseInt( $('#max_id').val(), 10 );
